    ...
    if option == 1:
        for i, mod in enumerate(features_files):
            ...
            for j, snr in enumerate(snr_list):
                for feature in range(number_of_features):
                    ...
                    plt.savefig(join(fig_folder, 'histogram',
                                     'histogram_specific_{}_{}dB_{}'.format(feature, snr_values[snr],
                                                                            mod.split('_')[0]) + '.png'),
                                bbox_inches='tight', dpi=300)
                    plt.clf()

                    report.write(
                        "Feature:{} - SNR:{}db - Max:{} - Min:{}\n".format(feature, snr_values[snr], str(max(ft)),
                                                                           str(min(ft))))
        ...
        for i, mod in enumerate(features_files):
            ...
            for j, snr in enumerate(snr_list):
                ...
                plt.savefig(join(fig_folder, 'histogram', 'histogram_all_{}_{}dB_{}'.format(feature, snr_values[snr],
                                                                                            mod.split('_')[
                                                                                                0]) + '.png'),
                            bbox_inches='tight', dpi=300)
                plt.clf()
        ...
        for feature in range(number_of_features):
            ...
            for j, snr in enumerate(snr_list):
                ...
                plt.savefig(join(fig_folder, 'histogram',
                                 'histogram_comparison_{}_{}dB'.format(feature, snr_values[snr]) + '.png'),
                            bbox_inches='tight', dpi=300)
                # plt.plot()
                # plt.show()
                plt.clf()
...
if __name__ == '__main__':
    ...
    # Option 2 will create a graphic with all features X snr X modulation
